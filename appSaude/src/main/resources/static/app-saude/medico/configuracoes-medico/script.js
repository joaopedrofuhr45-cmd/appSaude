...
async function carregarPerfil() {
    if (!usuario?.id) {
        mostrarMensagem(mensagemPerfil, "Não foi possível identificar o médico logado.", true);
        return;
    }

    try {
        const perfil = await medicoService.getPerfil(usuario.id);
...
document.querySelector("#form-perfil").addEventListener("submit", async (event) => {
    ...
    try {
        await medicoService.atualizarPerfil(usuario.id, {
...
document.querySelector("#form-senha").addEventListener("submit", async (event) => {
    ...
    try {
        await medicoService.atualizarSenha(usuario.id, senhaAtualInput.value, novaSenhaInput.value);
